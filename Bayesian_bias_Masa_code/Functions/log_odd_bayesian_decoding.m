...
% bayesian_spike_count: Maybe original replay spike count (common good
% cells) or rate-fixed replay spike count (common good cells)
% place_field_index: Stable common good cells (based on subsets_of_cells)
% Save_option: enter 'Y' for saving. Else, won't save.
% OUTPUTS:
% estimated_position structure.


% load extracted_place_fields_BAYESIAN (Don't load it)
parameters=list_of_parameters;
load extracted_position
load decoded_track_replay
...
elseif strcmp(bayesian_spike_count, 'replayEvents_common_good_cell_bayesian_spike_count')
    ...
    % elseif strcmp(bayesian_spike_count,'replayEvents_FIXED_spike_count_one_bin_median_spike_count')
    %     cd 'rate_fixed'
    %     load replayEvents_FIXED_spike_count_one_bin_median_spike_count
...
%         Track 1 and Track 2 place fields concatenated into one
    ...
    elseif strcmp(modify,'PRE place_field_circular_shift')
        % for circular shift, get place fields for each event
        for event = 1:length(bayesian_spike_count.replay_events)
            for track_id = 1:2
                for k=1:length(place_field_index)
                    single_place_field = place_fields_BAYESIAN.track(track_id).circular_shifted_raw{event}{place_field_index(k)}; %get raw place field
                    single_place_field(find(isnan(single_place_field))) = 0; % remove NaNs in place field and replace by 0
                    % single_place_field = smooth(single_place_field,parameters.smoothing_number_of_bins); %smooth place field
    ...
    elseif strcmp(modify,'PRE normal_place_field_circular_shift')
        % for circular shift, get place fields for each event
        for event = 1:length(bayesian_spike_count.replay_events)
            
            for track_id = 1:2
                for k=1:length(place_field_index)
                    single_place_field = place_fields_BAYESIAN.track(track_id).normal_circular_shifted_raw{event}{place_field_index(k)}; %get raw place field
                    single_place_field(find(isnan(single_place_field))) = 0; % remove NaNs in place field and replace by 0
                    % single_place_field = smooth(single_place_field,parameters.smoothing_number_of_bins); %smooth place field
    ...
    elseif strcmp(modify,'global remapping')
        ...
        for event = 1:length(bayesian_spike_count.replay_events)
            for track_id = 1:2
                for k=1:length(place_field_index)
                    single_place_field = place_fields_BAYESIAN.track(track_id).global_remapped_raw{event}{place_field_index(k)}; %get raw place field
                    single_place_field(find(isnan(single_place_field))) = 0; % remove NaNs in place field and replace by 0
                    % single_place_field = smooth(single_place_field,parameters.smoothing_number_of_bins); %smooth place field
    ...
    if strcmp(modify,'global remapping') | strcmp(modify,'PRE place_field_circular_shift') | strcmp(modify,'PRE normal_place_field_circular_shift')
...
if strcmp(modify,'PRE place_field_circular_shift') | strcmp(modify,'global remapping')...
        | strcmp(modify,'PRE normal_place_field_circular_shift') | strcmp(modify,'rate remapping')
    if isempty(option)
        estimated_position(1).replay = track_reconstruct(n.replay,all_place_fields{1},place_field_index,replay_id,bin_width,'per event');
    elseif strcmp(option,'ratemap shuffle')
        estimated_position(1).replay = track_reconstruct(n.replay,all_place_fields{2},place_field_index,replay_id,bin_width,'per event');
        
    end
else
    if isempty(option)
        estimated_position(1).replay = track_reconstruct(n.replay,all_place_fields{1},place_field_index,replay_id,bin_width,'');
    elseif strcmp(option,'ratemap shuffle')
        estimated_position(1).replay = track_reconstruct(n.replay,all_place_fields{2},place_field_index,replay_id,bin_width,'');
    end
end
...
if isfield(bayesian_spike_count,'replay_events')
    for track_id = 1
        for event = 1 : length(decoded_track_replay(1).replay_events)
            ...
            % Calculate ((sum of probability of T1)/(sum of probability of T2))
