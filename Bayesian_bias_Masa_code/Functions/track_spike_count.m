...
function bayesian_spike_count = track_spike_count(place_fields_BAYESIAN,start_time,end_time,timebin,place_field_index,save_option)
...
elseif size(start_time,2)>1 && size(end_time,2)>1 %if there's a vector for start and end times
    ...
    if isempty(timebin)
        for i = 1: length(start_time)
            ...
            t.event_duration{i} = event_duration;
        end 
%         t.run_edges = []; % don't want to run this if just checking replay events
%     elseif strcmp(timebin,'whole')
%         for i = 1: length(start_time)
%             event_duration = end_time(i) - start_time(i);
%             replay_time_edges{i} = [start_time(i) end_time(i)];
%             t.event_duration{i} = event_duration
%         end
    elseif isnumeric(timebin) && timebin >= 1 % Specify the number of evenly spaced time bin instead of ms length
        for i = 1: length(start_time)
            event_duration = end_time(i) - start_time(i);
            num_bins = timebin;
            timebins_edges = linspace(start_time(i), end_time(i), num_bins+1);
            replay_time_edges{i} = timebins_edges;     
            t.event_duration{i} = event_duration;
        end        
    else % if less than 1, then assume that it is ms timebin (but 50 ms seems to be its upper limit)
        for i = 1: length(start_time)
            event_duration = end_time(i) - start_time(i);
            num_bins = ceil(event_duration / timebin);
            timebins_edges = linspace(start_time(i), start_time(i) + num_bins *  timebin, num_bins+1);
            replay_time_edges{i} = timebins_edges;
        end
    end
...
%         bayesian_spike_count.replay_events(i).event_duration = t.event_duration{i};
...
%         elseif strcmp(timebin,'whole')
%             bayesian_spike_count.replay_events(i).replay_time_centered = mean(t.replay_edges); %centres of bins
        elseif isnumeric(timebin) && timebin >= 1
            for n = 1:length(t.replay_edges)-1
                bayesian_spike_count.replay_events(i).replay_time_centered(n) = mean(t.replay_edges(n)+t.replay_edges(n+1)); %centres of bins
                
            end
                        
        else
            bayesian_spike_count.replay_events(i).replay_time_centered = t.replay_edges(1:end-1)+timebin/2; %centres of bins
        end        
