...
for f = 1:length(folders)
    ...
    idx_subset = find(cell2mat(arrayfun(@(x) strcmp(subset_of_cells.subset(x),'stable cells laps'),1:size(subset_of_cells,1),'UniformOutput',0))); % MH:hope you don't mind me adding this, just in case the idx value ever changes
    place_cell_index = subset_of_cells.cell_IDs{idx_subset}...
        (intersect(find(subset_of_cells.cell_IDs{idx_subset} >= 1000*f),find(subset_of_cells.cell_IDs{idx_subset} <= 1000*(f+1))))- f*1000;
    ...
    % Save all replay events all tracks (decoded_track_replay is no longer important)
    decoded_track_replay(1).replay_events = replay_events; % Original Track
    % decoded_track_replay(2).replay_events = replay_events; % Shuffled Track
    
    decoded_track_replay(1).replay_active_cells = [];
    % decoded_track_replay(2).replay_active_cells = [];
    
    for i = 1:length(replay_events)
        if ~isempty(replay_events(i).spikes)
            decoded_track_replay(1).replay_active_cells{i} = unique(replay_events(i).spikes(:,1));
            %         decoded_track_replay(2).replay_active_cells{i} = unique(replay_events(i).spikes(:,1));
        end
    end
    ...
    if isnumeric(posbin) && ~isempty(posbin) % if time bin is not 20ms, make 'Track' Field
        ...
        for event = 1:length(a.replay_events)
            for track_id = 1:2
                ...
                for j = 1 : length(place_fields_BAYESIAN(1).raw)
...
%         
%         for track_id = 1:2
%             original_cell = place_fields_BAYESIAN.id_conversion(track_id).original_good_cells;
%             random_cell = place_fields_BAYESIAN.id_conversion(track_id).permuted_good_cells;
%             Track_fields(track_id).global_remapped_mean_rate = Track_fields(track_id).mean_rate;
%             for j = 1 : length(random_cell) %only swap good cells
%                 Track_fields(track_id).global_remapped_mean_rate(:,original_cell(j)) = Track_fields(track_id).mean_rate(:,random_cell(j));
%             end
%         end
