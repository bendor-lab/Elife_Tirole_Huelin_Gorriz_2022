...
option = {'original','rate fixed','original','rate fixed global remapping','rate remapped one bin','rate fixed'};
time_bin_option = [0.02,0.02,1,0.02,1,1];
ROC = [];
...
pvalue{4}',Mean{5}',SE{5}',pvalue{5}',Mean{6}',SE{6}',pvalue{6}')
...
Mean_AUC{6}',SE_AUC{6}',UCI_AUC{6}',LCI_AUC{6}')
...
%% 95% Confidence Interval to get P value
AUC = [];
AUC_UCI = [];
AUC_LCI = [];
...
for epoch = 1:3
    for n = 1:5
        AUC(epoch,n) = mean(AUC_bootstrap{epoch}{n});
        AUC_UCI(epoch,n) = prctile(AUC_bootstrap{epoch}{n},97.5);
        AUC_LCI(epoch,n) = prctile(AUC_bootstrap{epoch}{n},2.5);
        AUC_SE(epoch,n) = std(AUC_bootstrap{epoch}{n});
    end 
end

[pvalue_versus_original pvalue_versus_shuffle] = CI_to_pvalue(log_odd,AUC,AUC_UCI,AUC_LCI)
