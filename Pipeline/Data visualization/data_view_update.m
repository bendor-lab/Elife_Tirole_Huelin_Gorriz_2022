...
%         data.multiunit_activity.timestamp=data.multiunit_activity.timestamp-time.rezero_time;
    ...
    if exist('extracted_place_fields_BAYESIAN.mat')==2
        disp('loading place_fields')
        load('extracted_place_fields_BAYESIAN.mat');
        data.place_fields=place_fields_BAYESIAN;
    else data.clusters=[];
    end
...
function raster_plot(x,y,c,h,varargin)
x2(1:3:length(x)*3)=x;
x2(2:3:length(x)*3)=x;
...
if isempty(c)
    ...
    if size(c,1)>1
        unique_y= unique(y,'stable');
        cell_ids= unique(varargin{1},'stable');
        for jj=1:length(unique_y)
            % find indices
%             disp('cell_plotted')
            cell_idx= cell_ids(jj);
            spk_idx_this_cell= min(find(y2==unique_y(jj))):max(find(y2==(unique_y(jj)+h)));
            plot(x2(spk_idx_this_cell),y2(spk_idx_this_cell),'Color',c(cell_idx,:),'LineWidth',2)
        end
    else
...
%   plot(data.replay_events_ripple(min(index):max(index)),.1+y.bottom(end)+ones(size(min(index):max(index))),'r','LineWidth',2)
 ...
 y.text{end+1}='LFP';
  ...
  c=parameters.plot_color_line;
    if exist('example_neuron_cdata.mat')
        load('example_neuron_cdata.mat');
        c2= plot_color_line;
    else
        c2= c;
    end
    ...
    for i=1:length(data.position.linear)
        y.bottom(end+1)=y.top(end)+0.5;
        number_of_units=0.7*max(data.place_fields.track(i).sorted_good_cells);
...
%             disp('cell_id_sorted')
%             data.place_fields.track(i).sorted_good_cells(j)
        ...
        raster_plot(spike_times_sorted,y.bottom(end)+10*spike_id_sorted/number_of_units,c2{i},9/number_of_units,spike_id_sorted);
    end
%  
if option~=2
y.bottom(end+1)=y.top(end)+0.5;
        ...
        raster_plot(spike_times_sorted,y.bottom(end)+10*spike_id_sorted/number_of_units,'k',9/number_of_units);
end
...
for i=1:number_of_tracks
    ...
    if time.all_time<=10 & ~isempty(data.estimated_position)
      ...
      lower_color_limit=(1./size(estimated_position,1));
        ...
        colormap(reverse_bone);
         plot3(data.position.t(p_index),100*data.position.linear(i).linear(p_index),ones(1,length(p_index)),'Color',c{i},'linewidth',1)
    elseif time.all_time<=300 & ~isempty(data.estimated_position)
        ...
        plot(data.position.t(p_index),100*data.position.linear(i).linear(p_index),'Color',c{i},'linewidth',1);
        hold on
        plot([time.min_time time.max_time],[0 0],'w.')
    end
