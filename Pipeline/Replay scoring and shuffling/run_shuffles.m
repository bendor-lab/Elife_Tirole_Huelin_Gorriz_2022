function shuffled_track = run_shuffles(shuffle_choice,analysis_type,num_shuffles,decoded_replay_events,varargin)
...
if strcmp(shuffle_choice, 'PRE spike_train_circular_shift')
    ...
    elseif ~isempty(varargin)
        switch varargin{1}
            case 'control_fixed_spike'
                option= 2;
                load('replayEvents_FIXED_spike.mat');
                replayEvents_bayesian_spike_count= replayEvents_FIXED_spike;
                replay_indices = replayEvents_FIXED_spike.replay_events_indices;
                spike_count_structure = replayEvents_FIXED_spike;
        end
    end
end
...
for s=1:num_shuffles
    ...
    elseif strcmp(shuffle_choice, 'PRE place_field_circular_shift')
        ...
        elseif ~isempty(varargin) &&  option==2
            estimated_position = bayesian_decoding(shuffled_place_fields,'replayEvents_FIXED_spike','N');
        end
