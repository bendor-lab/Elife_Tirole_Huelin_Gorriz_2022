...
p = inputParser;
addParameter(p,'direction','bidirectional',@ischar);
addParameter(p,'grouping','each',@ischar);
...
str_name2= [str_name2 '.mat'];
    ...
    if ~strcmp(p.Results.grouping,'even_odd')
        length_laps= cell2mat(cellfun(@(x) length(x),previous_laps,'UniformOutput',false));
    ...
    if p.Results.save_option
        save(str_name,'place_field_laps','-v7.3');
    end
