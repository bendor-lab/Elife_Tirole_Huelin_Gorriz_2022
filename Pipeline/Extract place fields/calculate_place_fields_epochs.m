...
if ~isempty(varargin)
    if strcmp(varargin{1},'global_remap') == 1 && x_bins_width == parameters.x_bins_width_bayesian
        load('extracted_place_fields_BAYESIAN.mat');
        place_fields_GLOBAL_REMAPPED= place_fields_BAYESIAN;
        clear place_fields_BAYESIAN;
    elseif strcmp(varargin{1},'global_remap') == 1 && x_bins_width == parameters.x_bins_width
        load('extracted_place_fields.mat');
        place_fields_GLOBAL_REMAPPED= place_fields;
        clear place_fields;
    elseif strcmp(varargin{1},'rate_remap') == 1 && x_bins_width == parameters.x_bins_width_bayesian
        load('extracted_place_fields_BAYESIAN.mat');
        place_fields_RATE_REMAPPED= place_fields_BAYESIAN;
        clear place_fields_BAYESIAN;
    elseif strcmp(varargin{1},'rate_remap') == 1 && x_bins_width == parameters.x_bins
        load('extracted_place_fields.mat');
        place_fields_RATE_REMAPPED= place_fields;
        clear place_fields;
    end  
else
...
for track_id= 1:length(position.linear)
    ...
    if ~isempty(varargin)
        if strcmp(varargin{1},'global_remap')
            ...
            random_cell= place_fields_GLOBAL_REMAPPED.id_conversion(track_id).permuted_good_cells;
            original_cell= place_fields_GLOBAL_REMAPPED.id_conversion(track_id).original_good_cells;
            place_fields_REMAPPED= place_fields;
            
            for j = 1 : length(random_cell) %only swap good cells
                place_fields_REMAPPED.track(track_id).spike_hist{original_cell(j)} = place_fields.track(track_id).spike_hist{random_cell(j)};
                place_fields_REMAPPED.track(track_id).raw{original_cell(j)} = place_fields.track(track_id).raw{random_cell(j)};

                place_fields_REMAPPED.track(track_id).smooth{original_cell(j)}         = place_fields.track(track_id).smooth{random_cell(j)};
                place_fields_REMAPPED.track(track_id).centre_of_mass(original_cell(j)) = place_fields.track(track_id).centre_of_mass(random_cell(j));
                place_fields_REMAPPED.track(track_id).peak(original_cell(j)) =  place_fields.track(track_id).peak(random_cell(j));
                place_fields_REMAPPED.track(track_id).centre(original_cell(j)) =  place_fields.track(track_id).centre(random_cell(j));
                place_fields_REMAPPED.track(track_id).raw_peak(original_cell(j)) =  place_fields.track(track_id).raw_peak(random_cell(j));

                place_fields_REMAPPED.track(track_id).mean_rate_session(original_cell(j)) = place_fields.track(track_id).mean_rate_session(random_cell(j));
                place_fields_REMAPPED.track(track_id).mean_rate_track(original_cell(j)) = place_fields.track(track_id).mean_rate_track(random_cell(j));
                place_fields_REMAPPED.track(track_id).half_max_width(original_cell(j))=place_fields.track(track_id).half_max_width(random_cell(j));

                place_fields_REMAPPED.track(track_id).skaggs_info(original_cell(j)) = place_fields.track(track_id).skaggs_info(random_cell(j));
                place_fields_REMAPPED.track(track_id).non_visited_bins = place_fields.track(track_id).non_visited_bins;
            end
            
             place_fields_REMAPPED.track(track_id).good_cells= place_fields_GLOBAL_REMAPPED.track(track_id).good_cells;
             place_fields_REMAPPED.track(track_id).good_cells_LIBERAL= place_fields_GLOBAL_REMAPPED.track(track_id).good_cells;

            % Sort new cells by place field centre
            [~,index] = sort(place_fields_REMAPPED.track(track_id).centre);
            place_fields_REMAPPED.track(track_id).sorted = index;
            [~,index] = sort(place_fields_REMAPPED.track(track_id).centre(place_fields_REMAPPED.track(track_id).good_cells));
            place_fields_REMAPPED.track(track_id).sorted_good_cells = place_fields_REMAPPED.track(track_id).good_cells(index);
            [~,index] = sort(place_fields_REMAPPED.track(track_id).centre(place_fields_REMAPPED.track(track_id).good_cells_LIBERAL));
            place_fields_REMAPPED.track(track_id).sorted_good_cells_LIBERAL = place_fields_REMAPPED.track(track_id).good_cells_LIBERAL(index);
             
            place_fields= place_fields_REMAPPED;
            clear place_fields_REMAPPED;
            
        elseif strcmp(varargin{1},'rate_remap')
            tracks_ids= find(~cellfun(@isempty,{place_fields.track.x_bin_centres}));
            number_of_rate_remapped_tracks = length(tracks_ids);
            number_of_neurons = length(place_fields.track(tracks_ids(1)).peak);
            
            for k = 1 : number_of_rate_remapped_tracks

                % Allocate variables
                place_fields_REMAPPED= place_fields;
               
                for j = 1 : number_of_neurons
                    scaling_factor= place_fields_RATE_REMAPPED.FR_scaling_factor(tracks_ids(k)).factor(j);
                    
                    % Multiply each spike and the raw peak FR by it's scaling factor
                    place_fields_REMAPPED.track(tracks_ids(k)).spike_hist{j} = scaling_factor*place_fields.track(tracks_ids(k)).spike_hist{j};
                    place_fields_REMAPPED.track(tracks_ids(k)).raw{j} = scaling_factor*place_fields.track(tracks_ids(k)).raw{j};

                    % Get place field information
                    place_fields_REMAPPED.track(tracks_ids(k)).smooth{j}         =  filtfilt(w,1,place_fields_REMAPPED.track(tracks_ids(k)).raw{j}); %smooth pl field
                    place_fields_REMAPPED.track(tracks_ids(k)).centre_of_mass(j) =  sum(place_fields_REMAPPED.track(tracks_ids(k)).smooth{j}.* place_fields_REMAPPED.track(tracks_ids(k)).x_bin_centres)/sum(place_fields_REMAPPED.track(tracks_ids(k)).smooth{j});  %averaged center
                    [place_fields_REMAPPED.track(tracks_ids(k)).peak(j) , index] =  max(place_fields_REMAPPED.track(tracks_ids(k)).smooth{j}); %peak of smoothed place field and index of peak (center)
                    if place_fields_REMAPPED.track(tracks_ids(k)).peak(j) ~=0
                        if length(index)>1 % very rare exception where you have multiple peaks of same height....
                            index= index(1);
                        end
                        place_fields_REMAPPED.track(tracks_ids(k)).centre(j) =  place_fields_REMAPPED.track(tracks_ids(k)).x_bin_centres(index);

                    else
                        place_fields_REMAPPED.track(tracks_ids(k)).centre(j) = NaN;
                    end
                    place_fields_REMAPPED.track(tracks_ids(k)).raw_peak(j)          = max(place_fields_REMAPPED.track(tracks_ids(k)).raw{j}); % raw pl field peak
                    place_fields_REMAPPED.track(tracks_ids(k)).mean_rate_session(j) = NaN; %mean firing rate
                    place_fields_REMAPPED.track(tracks_ids(k)).mean_rate_track(j)   = sum(place_fields_REMAPPED.track(tracks_ids(k)).spike_hist{j})/(place_fields_REMAPPED.track(tracks_ids(k)).time_window(2)-place_fields_REMAPPED.track(tracks_ids(k)).time_window(1));
                    if place_fields_REMAPPED.track(tracks_ids(k)).peak(j) ~=0
                        place_fields_REMAPPED.track(tracks_ids(k)).half_max_width(j) = x_bins_width*half_max_width(place_fields_REMAPPED.track(tracks_ids(k)).smooth{j}); %finds half width of smoothed place field (width from y values closest to 50% of peak)
                    else
                        place_fields_REMAPPED.track(tracks_ids(k)).half_max_width(j) = NaN;
                    end
...
if isempty(varargin)
    ...
    if strcmp(varargin{1},'global_remap')
        ...
        place_fields.good_place_cells_LIBERAL= place_fields_GLOBAL_REMAPPED.good_place_cells_LIBERAL;
        place_fields.unique_cells= place_fields_GLOBAL_REMAPPED.unique_cells;
        place_fields.unique_cells= place_fields_GLOBAL_REMAPPED.unique_cells;
        place_fields.pyramidal_cells= place_fields_GLOBAL_REMAPPED.pyramidal_cells;
        place_fields.other_cells= place_fields_GLOBAL_REMAPPED.other_cells;
    elseif strcmp(varargin{1},'rate_remap')
        place_fields.good_place_cells= place_fields_RATE_REMAPPED.good_place_cells;
        place_fields.good_place_cells_LIBERAL= place_fields_RATE_REMAPPED.good_place_cells_LIBERAL;
        place_fields.unique_cells= place_fields_RATE_REMAPPED.unique_cells;
        place_fields.unique_cells= place_fields_RATE_REMAPPED.unique_cells;
        place_fields.pyramidal_cells= place_fields_RATE_REMAPPED.pyramidal_cells;
        place_fields.other_cells= place_fields_RATE_REMAPPED.other_cells;
    end
end
