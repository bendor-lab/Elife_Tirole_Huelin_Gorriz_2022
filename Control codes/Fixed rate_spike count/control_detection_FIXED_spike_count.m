function control_detection_FIXED_spike_count(current_folder,RExp)
% works in separate folder (CONTROLS\fixed_spike_count)
% input list of folders to process
% and vector of reexposure options

if exist('..\CONTROLS\fixed_spike_count')~=7
    mkdir('..\CONTROLS\fixed_spike_count');
end

    
    if exist(['..\CONTROLS\fixed_spike_count\' current_folder])~=7
        mkdir(['..\CONTROLS\fixed_spike_count\' current_folder]);
    end
    
    % copy over files needed 
    copyfile('extracted_replay_events.mat',['..\CONTROLS\fixed_spike_count\' current_folder]);
    copyfile('decoded_replay_events.mat',['..\CONTROLS\fixed_spike_count\' current_folder]);
    copyfile('decoded_replay_events_segments.mat',['..\CONTROLS\fixed_spike_count\' current_folder]);
    copyfile('extracted_clusters.mat',['..\CONTROLS\fixed_spike_count\' current_folder]);
    copyfile('extracted_place_fields_BAYESIAN.mat',['..\CONTROLS\fixed_spike_count\' current_folder]);
    copyfile('extracted_position.mat',['..\CONTROLS\fixed_spike_count\' current_folder]);
    copyfile('replayEvents_bayesian_spike_count.mat',['..\CONTROLS\fixed_spike_count\' current_folder]);
    copyfile('extracted_sleep_state.mat',['..\CONTROLS\fixed_spike_count\' current_folder]);

    cd(['..\CONTROLS\fixed_spike_count\' current_folder]);    
    
    replay_decoding('control_fixed_count');
    ...
    plot_rate_remapping('TRACK_PAIRS','control_fixed_count');
