...
for this_folder= 1:length(folders)
    ...
    % get indices of replay events for this session and epoch
    if this_folder==2
        load(['significant_replay_events_' method '_individual_exposures.mat']);
    else
        load(['significant_replay_events_' method '.mat']);
    end
